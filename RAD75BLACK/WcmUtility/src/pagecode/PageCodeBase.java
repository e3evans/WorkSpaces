...
public abstract class PageCodeBase {
	...
	public static String SESS_PORTLET_PREFERENCES = "com_manpower_portal_portlet_wcmviewer_portletpreferences";
	...
	protected Workspace getWcmWorkSpace(){
		Workspace workspace = null;
		try {			
			PortletPreferences prefs = getPortletPreferences();
			workspace = getWcmRepository()
				.getWorkspace(prefs.getValue(WcmViewerConfig.CONFIG_PREF_WCMUSERNAME, ""),
	...
	/**
	 * Only good in context with actions....
	 * @param facescontext
	 * @return
	 */
	protected Map getFieldMap(FacesContext facescontext){
		ActionRequest req = (ActionRequest) facescontext.getExternalContext().getRequest();
		Map fieldMap = new HashMap();
		for (Enumeration e = req.getParameterNames();e.hasMoreElements();){
			String temp = (String)e.nextElement();
			//System.out.println("***"+temp);
			fieldMap.put(temp,req.getParameter(temp));
		}
		return fieldMap;
	}
}