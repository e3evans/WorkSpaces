...
final String METHOD_NAME="news.jsp";
...
<div id="<%=ns%>newsBuilder">
	<div>
<%org.eclipse.core.runtime.IConfigurationElement []styles=ice.getChildren("style");
if(styles.length>0){%>
		<label for="<%=ns%>newsStyle" title="<fmt:message key="news_builder_style_label_tip" />"><fmt:message key="news_builder_style_label" /></label><br />
		<select name="news_style" id="<%=ns%>newsStyle">
...
<%}%>
		<label for="<%=ns%>newsPageTitle" title="<fmt:message key="news_builder_title_label_tip" />"><fmt:message key="news_builder_title_label" /></label><br />
		<input type="text" name="news_title" id="<%=ns%>newsPageTitle" value="" dojoType="dijit.form.ValidationTextBox" trim="true" propercase="true" required="true" invalidMessage="<fmt:message key="news_builder_invalid_title_message" />" regExp="[^\s].*" />
	</div>
</div><%
