...
final String METHOD_NAME="annc.jsp";
...
<div id="<%=ns%>anncBuilder">
    <div>
<%org.eclipse.core.runtime.IConfigurationElement []styles=ice.getChildren("style");
if(styles.length>0){%>
        <label for="<%=ns%>anncStyle" title="<fmt:message key="annc_builder_style_label_tip" />"><fmt:message key="annc_builder_style_label" /></label><br />
        <select name="annc_style" id="<%=ns%>anncStyle">
...
<%}%>
        <label for="<%=ns%>anncPageTitle" title="<fmt:message key="annc_builder_title_label_tip" />"><fmt:message key="annc_builder_title_label" /></label><br />
        <input type="text" name="annc_title" id="<%=ns%>anncPageTitle" value="" dojoType="dijit.form.ValidationTextBox" trim="true" propercase="true" required="true" invalidMessage="<fmt:message key="annc_builder_invalid_title_message" />" regExp="[^\s].*" />
    </div>
</div><%
